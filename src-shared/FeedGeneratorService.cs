...
public class FeedGeneratorService
{
    ...
    public FeedGeneratorService(IHttpClientFactory httpClientFactory, string baseUrl, GeneratorConfig? config = null)
    {
    ...
    public async Task<int> GenerateAllFeedsAsync(string podcastsJsonPath)
    {
        ...
        WebsiteGenerator.Generate(feedMetadata, _config);

        return feedMetadata.Count;
    }

    private async Task<FeedMetadata?> ProcessPodcastAsync(Podcast podcast)
    {
        try
        {
            ...
            var (rss, metadata) = BuildRssFeed(series, episodes, podcast, _baseUrl);

            // Ensure feeds directory exists
            Directory.CreateDirectory(_config.FeedsDir);

            // Save directly to final location
            string outputPath = Path.Combine(_config.FeedsDir, $"{podcast.Slug}.xml");
    ...
    private static (XElement rss, FeedMetadata metadata) BuildRssFeed(Series? series, List<Episode>? episodes, Podcast podcast, string baseUrl)
    {
    ...
    private static async Task<List<Episode>?> FetchAllEpisodesAsync(string initialUrl, HttpClient httpClient)
    {
